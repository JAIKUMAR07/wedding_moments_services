            ...
            >
              <div className="flex gap-1 mb-4 text-amber-400">
                {[...Array(5)].map((_, i) => (
                  <Star
                    key={i}
                    className={`w-5 h-5 ${i < item.rating ? "fill-current text-amber-400" : "text-zinc-700 fill-current"}`}
                  />
                ))}
              </div>
              ...
              <div className="flex items-center gap-4">
                <div className="w-12 h-12 rounded-full bg-linear-to-br from-amber-400 to-orange-600 flex items-center justify-center text-white font-serif text-xl font-bold">
                  {item.name.charAt(0)}
                </div>
                <div>
